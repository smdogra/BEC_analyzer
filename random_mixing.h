...
void MixEvents_random(int ntrkoff_min, int ntrkoff_max, int nEvt_to_mix, std::vector<int> ev_ntrkoff_vec, std::vector<double> ev_vtx_z_vec, std::vector<std::vector<TLorentzVector>> ev_GoodTrackFourVector_vec, std::vector<std::vector<int>> ev_GoodTrackCharge_vec){
 
  int aux_n_evts = (int)ev_ntrkoff_vec.size(); // total number of events

  //TRandom2 *r = new TRandom2(); // random number producer
  cout << " I am inside of mixing process and the number of events to be mixed are  :    " <<  aux_n_evts <<endl;
  ...
  for(int nevt1=0; nevt1<aux_n_evts; nevt1++) {
    ...
    for(int nevt_assoc=nevt1+1; nevt_assoc<aux_n_evts; nevt_assoc++) {
      ...
      hist_DeltazVertex_->Fill((ev_vtx_z_vec)[nevt1] - (ev_vtx_z_vec)[nevt_assoc]);
	...
	for(int iimix=0; iimix<nMixmult_nevt_assoc; iimix++){
	  ...
	  Double_t aux_tk12_corr=1.; //just to compare with applying corrections 
	  if(ev_GoodTrackChargeTemp_nevt1_vec[imix]*ev_GoodTrackChargeTemp_nevt_assoc_vec[iimix]>0){

	     hist_mixsch_KtVsNch_ntrkoff->Fill(kt,ev_ntrkoff_vec[nevt1],aux_tk12_corr);
...
void call_mix_random(int ntrkoff_min, int ntrkoff_max, int nEvt_to_mix, std::vector<int> ev_ntrkoff_vec, std::vector<double> ev_vtx_z_vec, std::vector<std::vector<TLorentzVector>> ev_GoodTrackFourVector_vec, std::vector<std::vector<int>> ev_GoodTrackCharge_vec){  
  ...
  MixEvents_random(ntrkoff_min, ntrkoff_max, nEvt_to_mix, ev_ntrkoff_vec,ev_vtx_z_vec, ev_GoodTrackFourVector_vec, ev_GoodTrackCharge_vec);
